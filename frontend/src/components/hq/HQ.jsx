  ...
  return (
    <div className="max-w-md mx-auto card p-8 space-y-5 mt-6">
      <div className="flex items-center gap-3">
        <div className="p-2.5 rounded-lg border border-[#deffac]/30 text-[#deffac]"><Lock className="w-5 h-5" /></div>
        <div><h2 className="font-semibold text-lg">Quartier Generale</h2><p className="text-xs text-neutral-400">Area riservata ai manager</p></div>
      </div>
      <form onSubmit={submit} className="space-y-3">
        <label className="block space-y-1"><span className="label">Codice di accesso</span>
          <input type="password" autoComplete="off" autoFocus value={code} onChange={(e) => setCode(e.target.value)} className="field !py-2.5 font-mono text-sm" placeholder="••••••••" /></label>
      ...
      <p className="text-xs text-neutral-500 leading-relaxed">Il codice viene controllato dal server, mai dal browser. Dopo 5 tentativi sbagliati l’accesso si blocca per 10 minuti. Ogni tentativo finisce nella timeline, senza il codice che hai scritto.</p>
    </div>
  )
}

function Kpi({ label, value, note, tone = 'text-neutral-100' }) {
  return <div className="card p-4"><span className="label">{label}</span><div className={`text-2xl font-bold font-mono mt-1 ${tone}`}>{value}</div>{note && <p className="text-xs text-neutral-500 mt-1">{note}</p>}</div>
}
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        <Kpi label="Budget controllati" value={data.validations} note={data.last_validation_ts ? `ultima ${fmtTs(data.last_validation_ts)}` : 'nessuna ancora'} tone="text-[#deffac]" />
        ...
        <Kpi label="Registrazioni" value={data.registry.entries} note={data.registry.intact ? 'registro integro' : 'REGISTRO ALTERATO'} tone={data.registry.intact ? 'text-emerald-300' : 'text-red-300'} />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className={`card p-4 space-y-2 ${data.storage.volatile ? '!border-amber-500/40' : ''}`}>
          ...
          <p className={`text-xs leading-relaxed ${data.storage.volatile ? 'text-amber-300' : 'text-neutral-400'}`}>{data.storage.volatile && <AlertTriangle className="w-3 h-3 inline mr-1" />}{data.storage.note}</p>
        </div>
        <div className={`card p-4 space-y-2 ${data.hq_code_is_default ? '!border-amber-500/40' : ''}`}>
          ...
          <p className={`text-xs leading-relaxed ${data.hq_code_is_default ? 'text-amber-300' : 'text-emerald-300'}`}>{data.hq_code_is_default ? 'Il codice manager è ancora quello di partenza (QUANTO_1), scritto nella documentazione del progetto: chiunque lo conosca può entrare. Cambialo impostando QUANTO_HQ_CODE sul server.' : 'Codice manager personalizzato.'}</p>
          <p className={`text-xs ${data.registry.is_dev_key ? 'text-amber-300' : 'text-neutral-400'}`}>Firma del registro: chiave {data.registry.key_id}{data.registry.is_dev_key ? ' (di prova)' : ''}</p>
        </div>
        <div className="card p-4 space-y-2">
          ...
          <div className="flex items-end gap-1 h-16">
            ...
            {data.events_by_day.length === 0 && <p className="text-xs text-neutral-500 italic">Nessun evento.</p>}
          </div>
        </div>
      </div>

      <div className="card p-4 space-y-2">
        <div className="flex items-center justify-between"><span className="label">Ultimi eventi</span><button onClick={() => goto('timeline')} className="text-xs text-[#deffac] hover:underline">Timeline completa →</button></div>
        {data.recent_events.map((e) => (
          <div key={e.id} className="flex flex-wrap items-center gap-x-3 text-xs border-b border-neutral-900 py-1.5 last:border-0">
            <span className="font-mono text-[11px] text-neutral-500 w-32 shrink-0">{fmtTs(e.ts)}</span>
        ...
        {data.recent_events.length === 0 && <p className="text-xs text-neutral-500">Ancora nessuna operazione registrata.</p>}
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-4">
      <div className="card p-3 flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs text-neutral-500 ml-auto">{events.length} eventi</span>
      </div>
            ...
            <div key={e.id} className="relative">
              ...
              <button onClick={() => setOpen(open === e.id ? null : e.id)} className="w-full text-left card p-3 hover:border-neutral-600 transition">
                <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5">
                  <span className="text-[11px] font-mono text-neutral-500">{fmtTs(e.ts)}</span>
                  <span className="text-xs font-bold text-[#deffac]">{op?.title || e.op}</span>
                  <span className="text-[11px] text-neutral-500">{op?.area}</span>
                  <span className={`text-[11px] font-medium ${STATUS_TONE[e.status] || 'text-neutral-300'}`}>{STATUS_LABEL[e.status] || e.status}</span>
                  {e.duration_ms != null && <span className="text-[11px] font-mono text-neutral-600">{e.duration_ms} ms</span>}
                  <span className="text-[11px] font-mono text-neutral-600 ml-auto">{e.actor}</span>
                </div>
                <p className="text-xs text-neutral-300 mt-1">{e.summary}</p>
                {(e.project_id || e.bando_id) && <p className="text-[11px] font-mono text-neutral-500 mt-0.5">{e.project_id && `progetto ${e.project_id}`}{e.project_id && e.bando_id && ' · '}{e.bando_id && `bando ${e.bando_id}`}</p>}
              </button>
              {open === e.id && (
                <div className="card p-3 mt-1 space-y-2 !bg-neutral-950">
                  {op?.stages?.length > 0 && (
                    <div className="flex flex-wrap items-center gap-1.5 text-[11px] text-neutral-400">{op.stages.map((s, i) => <span key={s} className="flex items-center gap-1.5"><span className="px-1.5 py-0.5 rounded bg-neutral-900 border border-neutral-800">{i + 1}. {s}</span></span>)}</div>
                  )}
                  {e.details && <pre className="text-[11px] font-mono text-neutral-300 overflow-x-auto max-h-64">{JSON.stringify(e.details, null, 2)}</pre>}
...
function Operations({ ops, onShow }) {
  ...
  return (
    <div className="space-y-6">
      <p className="text-xs text-neutral-400">Tutto ciò che l’app sa fare, con i passaggi interni nell’ordine reale e quante volte è stato eseguito.</p>
      {areas.map(([area, list]) => (
            ...
            {list.map((o) => (
              <div key={o.id} className="card p-4 space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div><p className="font-bold text-sm">{o.title}</p><p className="text-[11px] font-mono text-neutral-500">{o.endpoint}</p></div>
                  <div className="text-right shrink-0"><p className="font-mono text-lg font-bold text-[#deffac]">{o.stats.count}</p><p className="text-[11px] text-neutral-500">esecuzioni</p></div>
                </div>
                <p className="text-xs text-neutral-400 leading-relaxed">{o.description}</p>
                <ol className="space-y-1">{o.stages.map((s, i) => (
                  <li key={s} className="flex items-center gap-2 text-xs text-neutral-300"><span className="w-4 h-4 rounded-full bg-neutral-800 text-[10px] font-bold flex items-center justify-center text-neutral-400 shrink-0">{i + 1}</span>{s}</li>))}</ol>
                <div className="flex flex-wrap items-center gap-3 text-[11px] font-mono text-neutral-500">
                  {o.criteria.length > 0 && <span className="text-sky-300">{o.criteria.length === 60 ? 'tutti i 60 controlli' : `controlli ${o.criteria.map((c) => `#${c}`).join(' ')}`}</span>}
                  {o.stats.avg_ms != null && <span>media {o.stats.avg_ms} ms</span>}
                  {o.stats.errors > 0 && <span className="text-amber-300">{o.stats.errors} con esito non riuscito</span>}
...
export default function HQ({ bandi, onReplay }) {
  ...
  return (
    <div className="space-y-6">
      <Guide page="hq" />
      <div className="flex flex-wrap items-center gap-3 border-b border-neutral-800">
        <div className="flex flex-wrap gap-x-5 gap-y-1 flex-1">
          {SECTIONS.map(([id, label]) => (
            <button key={id} onClick={() => goto(id, '')} className={`pb-2.5 text-sm -mb-px border-b-2 transition ${section === id ? 'border-[#deffac] text-white font-medium' : 'border-transparent text-neutral-400 hover:text-white'}`}>{label}</button>
          ))}
        </div>
