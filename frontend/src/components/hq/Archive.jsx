...
function TextViewer({ id, source, onClose }) {
  ...
  return (
    <div className="p-3 bg-neutral-950 border border-neutral-800 rounded-xl space-y-2">
      <div className="flex items-center gap-2"><span className="text-xs font-medium truncate">{source.name}</span><button onClick={onClose} className="ml-auto text-xs text-neutral-400 hover:text-white">Chiudi</button></div>
      {error && <p className="text-xs text-red-300">{error}</p>}
      {!data && !error && <p className="text-xs text-neutral-500 flex items-center gap-2"><Loader2 className="w-3 h-3 animate-spin" />Carico…</p>}
      {data && (<>
        ...
        {excerpts ? (
          <div className="space-y-2 max-h-96 overflow-y-auto">
            <p className="text-[11px] text-neutral-500">{excerpts.length} passaggi con “{q.trim()}”</p>
...
function Documents({ id, sources, onChanged }) {
  ...
  if (!sources.length) return <p className="text-xs text-neutral-500">Nessun documento in memoria per questo bando.</p>
  return (
    <div className="space-y-2">
      {error && <p className="text-xs text-red-300">{error}</p>}
      <p className="text-xs text-neutral-500">{sources.length} documenti · {fmtBytes(sources.reduce((a, s) => a + (s.file?.size_bytes || 0), 0))} di file originali conservati</p>
      {sources.map((s) => {
        const issues = [...(s.warnings || []), ...(s.analysis?.note ? [s.analysis.note] : [])]
        return (
          <div key={s.sha256} className="p-3 bg-neutral-950 border border-neutral-800 rounded-xl space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              {s.tier ? <span className={`px-1.5 py-0.5 rounded border text-[11px] ${TIER_STYLE[s.tier]}`}>{s.tier === 'UFFICIALE' ? 'ufficiale' : 'secondaria'}</span> : <span className="px-1.5 py-0.5 rounded border border-neutral-700 text-[11px] text-neutral-400">caricato a mano</span>}
              {s.url ? <a href={s.url} target="_blank" rel="noreferrer" className="text-sky-300 hover:underline inline-flex items-center gap-1 text-xs truncate max-w-full">{s.name}<ExternalLink className="w-3 h-3 shrink-0" /></a> : <span className="text-xs">{s.name}</span>}
            </div>
            <p className="text-[11px] text-neutral-500">{s.content_type || '—'}{s.pages ? ` · ${s.pages} pag.` : ''} · {s.chars.toLocaleString('it-IT')} caratteri{s.analysis?.lang ? ` · lingua ${s.analysis.lang}` : ''} · {s.analysis ? `${s.analysis.requirements} requisiti` : 'non ancora letto'} · {fmtTs(s.ts)}</p>
            {issues.map((w) => <p key={w} className="text-[11px] text-amber-300 flex items-start gap-1"><AlertTriangle className="w-3 h-3 shrink-0 mt-0.5" />{w}</p>)}
            <div className="flex flex-wrap gap-2">
              ...
              <button onClick={() => remove(s)} className="btn !py-1 !text-red-300 !border-red-500/30 ml-auto"><Trash2 className="w-3 h-3" />Elimina</button>
            </div>
...
function Rules({ id, detail, sheet, onChanged }) {
  ...
  return (
    <div className="space-y-3">
      {error && <p className="text-xs text-red-300">{error}</p>}
      <p className="text-xs text-neutral-500">Qui imposti tu il valore: vale come decisione del manager e diventa un controllo attivo. Formati: numero (35), percentuale come 0.15 oppure 15%, true/false, elenco JSON come ["CAPITAL_ASSETS"].</p>
      {detail.rules.map((r) => (
        <div key={r.key} className="p-3 bg-neutral-950 border border-neutral-800 rounded-xl space-y-1.5">
          <div className="flex flex-wrap items-center gap-2 text-xs"><span className="font-mono text-neutral-100">{r.key}</span><span className="text-neutral-500">{labels[r.key]}</span>
            <span className={`px-1.5 rounded border text-[10px] ${r.status === 'PUBLISHED' ? 'border-emerald-500/30 text-emerald-300' : 'border-amber-500/30 text-amber-300'}`}>{r.status === 'PUBLISHED' ? 'attiva' : 'da decidere'}</span>
            <span className="text-neutral-600">{r.origin}</span></div>
          {r.status !== 'PUBLISHED' && r.passes && <p className="text-[11px] text-amber-300">Letture diverse: {r.passes.join(' ≠ ')} — scegli quella giusta:{' '}
            {r.passes.map((p) => <button key={p} onClick={() => save(r.key, p)} className="btn !py-0.5 !px-2 mr-1">{p}</button>)}</p>}
          {r.source_ref && <p className="text-[11px] text-neutral-500 break-words">Fonte: {r.source_ref}</p>}
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={() => remove(r.key)} className="btn !text-red-300 !border-red-500/30"><Trash2 className="w-3 h-3" />Elimina</button>
          </div>
        </div>
      ))}
      {detail.rules.length === 0 && <p className="text-xs text-neutral-500">Nessuna regola.</p>}
      <div className="p-3 border border-neutral-800 rounded-xl space-y-2">
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <span className="text-xs text-neutral-500 self-center">{rows.length} di {items.length}</span>
      </div>
      {rows.slice(0, limit).map((r) => (
        <div key={r.seq} className="p-3 bg-neutral-950 border border-neutral-800 rounded-xl space-y-1.5 text-xs">
          <div className="flex flex-wrap items-center gap-2"><span className="font-medium">{r.topic}</span>{r.criteria.map((c) => <span key={c} className="font-mono text-[11px] text-[#deffac]">#{c}</span>)}
            ...
            <button onClick={() => remove(r)} aria-label="Elimina requisito" className="text-neutral-500 hover:text-red-300"><Trash2 className="w-3.5 h-3.5" /></button></div>
...
function Fold({ title, count, tone = '', defaultOpen = false, children }) {
  ...
  return (
    <div className="border border-neutral-800 rounded-xl">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center gap-2 px-3 py-2.5 text-left text-sm font-medium">{open ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}<span className={tone}>{title}</span><span className="ml-auto text-xs text-neutral-500 font-mono">{count}</span></button>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="p-3 bg-neutral-950 border border-neutral-800 rounded-xl space-y-2">
        ...
        <ol className="space-y-1.5 text-xs text-neutral-300 list-decimal pl-5">{sheet.checklist.map((c) => <li key={c} className="leading-relaxed">{c}</li>)}</ol>
        <p className="text-[11px] text-neutral-500">{s.rules_published} regole attive · {s.conflicts} da decidere · {s.silent} non trovate · {s.review} requisiti da rivedere · {s.document_issues} documenti con problemi</p>
      </div>

      <Fold title="Da decidere: documenti in disaccordo" count={sheet.conflicts.length} tone="text-amber-300" defaultOpen={sheet.conflicts.length > 0}>
        {sheet.conflicts.length === 0 && <p className="text-xs text-neutral-500">Nessuna regola in disaccordo.</p>}
        {sheet.conflicts.map((c) => (
          <div key={c.key} className="p-3 bg-neutral-950 border border-neutral-800 rounded-xl space-y-1.5 text-xs">
            <p className="font-medium">{c.label} <span className="font-mono text-neutral-500">{c.key}</span></p>
            <p className="text-neutral-400 leading-relaxed">{c.what_to_do}</p>
            <p className="text-neutral-500 break-words">Fonti: {c.source}</p>
      ...
      <Fold title="Cosa dice il bando (regole attive con la fonte)" count={sheet.says.length}>
        {sheet.says.map((x) => (
          <div key={x.key} className="p-3 bg-neutral-950 border border-neutral-800 rounded-xl text-xs space-y-1">
            <p className="font-medium">{x.label}: <span className="font-mono text-[#deffac]">{JSON.stringify(x.value)}</span> <span className="text-neutral-500">· controlli {x.criteria.map((n) => `#${n}`).join(' ')}</span></p>
            <p className="text-neutral-500 break-words">Fonte: {x.source || '—'} · affidabilità {x.confidence}</p>
            <p className="text-neutral-400">{x.check}</p>
          </div>
        ))}
        {sheet.says.length === 0 && <p className="text-xs text-neutral-500">Nessuna regola attiva.</p>}
      </Fold>

      <Fold title="Cosa NON dice: regole non trovate, cosa cercare e come verificare" count={sheet.silent.length} defaultOpen>
        <p className="text-xs text-neutral-500 leading-relaxed">Ogni riga è una regola che il motore sa usare ma che non ho trovato nei documenti. Finché non c’è, i controlli collegati restano spenti (mai “superati”). Per ognuna trovi cosa fa, quali parole cercare, dove compare di solito e come verificare.</p>
        {sheet.silent.map((x) => (
          <details key={x.key} className="border border-neutral-800 rounded-xl group">
            <summary className="cursor-pointer list-none px-3 py-2 flex flex-wrap items-center gap-2 text-xs">
              <span className="font-medium">{x.label}</span>
              <span className={`px-1.5 rounded border text-[10px] ${x.mentions ? 'border-amber-500/30 text-amber-300' : 'border-neutral-700 text-neutral-500'}`}>{x.mentions ? `${x.mentions} citazioni nei documenti` : 'mai citata'}</span>
              <span className="ml-auto font-mono text-[10px] text-neutral-600">{x.criteria.map((c) => `#${c.n}`).join(' ')}</span>
            </summary>
            <div className="px-3 pb-3 space-y-2 text-xs text-neutral-300">
              <p><span className="text-neutral-500">Cosa significa · </span>{x.meaning}</p>
              {x.criteria.length > 0 && <p><span className="text-neutral-500">Controlli spenti · </span>{x.criteria.map((c) => `#${c.n} ${c.title}`).join(' · ')}</p>}
              <p><span className="text-neutral-500">Stato · </span>{x.state}</p>
              <p className="break-words leading-7"><span className="text-neutral-500">Cerca · </span>{x.search.map((w) => <code key={w} className="mr-1.5 px-1 rounded bg-neutral-800 whitespace-normal break-all">{w}</code>)}</p>
              <p><span className="text-neutral-500">Dove · </span>{x.where}</p>
              <p><span className="text-neutral-500">Come verificare · </span>{x.verify}</p>
              {x.snippets.map((sn) => <p key={sn} className="border-l-2 border-neutral-700 pl-3 text-neutral-400">{sn}</p>)}
      ...
      <Fold title="Requisiti da rivedere (obblighi e divieti non classificati)" count={sheet.review.length}>
        {sheet.review.map((r) => (
          <div key={r.seq} className="p-3 bg-neutral-950 border border-neutral-800 rounded-xl text-xs space-y-1.5">
            <p className="text-neutral-300 leading-relaxed">{r.text}</p>
            <p className="text-[11px] text-neutral-500 break-words">Fonte: {r.source_ref}</p>
        ...
        {sheet.review.length === 0 && <p className="text-xs text-neutral-500">Nessun requisito da rivedere.</p>}
      </Fold>
      ...
      <Fold title="Documenti letti" count={sheet.documents.length}>
        {sheet.documents.map((d) => (
          <p key={d.sha256} className="text-xs flex flex-wrap items-center gap-2">
            {d.issues.length ? <AlertTriangle className="w-3.5 h-3.5 text-amber-300 shrink-0" /> : <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />}
            <span className="text-neutral-200">{d.name}</span><span className="text-neutral-500">{d.requirements ?? '—'} requisiti{d.lang ? ` · ${d.lang}` : ''}</span>
            {d.issues.map((w) => <span key={w} className="text-amber-300 w-full pl-5">{w}</span>)}
          </p>
        ))}
      </Fold>
    </div>
...
function BandoManager({ id, deletedDefaults, onList }) {
  ...
  if (!detail || !sheet) return <p className="text-xs text-neutral-500 flex items-center gap-2"><Loader2 className="w-3.5 h-3.5 animate-spin" />Carico…</p>
  ...
  return (
    <div className="card p-5 space-y-4">
      <div><h3 className="font-semibold text-lg break-words">{detail.name}</h3><p className="text-xs font-mono text-neutral-500">{detail.bando_id} · {detail.extraction_status}</p></div>
      <div className="flex items-center gap-3"><div className="flex flex-wrap gap-x-5 gap-y-1 border-b border-neutral-800 flex-1">
        {tabs.map(([k, l]) => <button key={k} onClick={() => setTab(k)} className={`pb-2 text-xs -mb-px border-b-2 ${tab === k ? 'border-[#deffac] text-white font-medium' : 'border-transparent text-neutral-400 hover:text-white'}`}>{l}</button>)}</div>
      ...
      {tab === 'actions' && (
        <div className="space-y-4">
          {actErr && <p className="text-xs text-red-300">{actErr}</p>}
          ...
          <p className="text-xs text-neutral-500">“Rileggi tutto” ricalcola regole automatiche e requisiti da tutti i documenti in memoria (le decisioni che hai preso a mano restano). L’esportazione contiene la scheda JSON, i testi estratti e i file originali.</p>
          ...
          <div className="pt-3 border-t border-neutral-800 space-y-2">
            <button onClick={remove} disabled={busy === 'del'} className="btn !text-red-300 !border-red-500/30"><Trash2 className="w-3.5 h-3.5" />Elimina il bando</button>
            <p className="text-xs text-neutral-500">Cancella documenti, file originali, regole e requisiti. La cronologia degli eventi resta. I bandi predefiniti si possono ripristinare.</p>
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-6 items-start">
      <div className="space-y-3">
        ...
        {error && <p className="text-xs text-red-300">{error}</p>}
        {list.map((b) => (
          <button key={b.bando_id} onClick={() => setSelected(b.bando_id)} className={`w-full text-left card p-3 space-y-1 hover:border-neutral-600 ${selected === b.bando_id ? '!border-[#deffac]' : ''}`}>
            <p className="text-sm font-medium leading-snug">{b.name}</p>
            <p className="text-[11px] text-neutral-500">{ORIGIN_LABEL[b.origin]} · {b.sources} documenti{b.files_bytes ? ` (${fmtBytes(b.files_bytes)})` : ''} · {b.rules} regole · {b.requirements} requisiti</p>
            {(b.rules_pending > 0 || b.requirements_to_review > 0) && <p className="text-[11px] text-amber-300 flex items-center gap-1"><AlertTriangle className="w-3 h-3" />{b.rules_pending ? `${b.rules_pending} da decidere` : ''}{b.rules_pending && b.requirements_to_review ? ' · ' : ''}{b.requirements_to_review ? `${b.requirements_to_review} da rivedere` : ''}</p>}
          </button>
        ))}
        {list.length === 0 && <p className="text-xs text-neutral-500">Nessun bando.</p>}
      ...
      <div className="lg:col-span-2 min-w-0">
        {selected ? <BandoManager id={selected} deletedDefaults={data.deleted_defaults} onList={load} />
          : <div className="card p-10 text-center text-sm text-neutral-500">Scegli un bando per vedere tutto ciò che è in memoria, correggerlo o eliminarlo.</div>}
      </div>
    </div>
  )
}
