    ...
    <div className="space-y-6">
      ...
      <div className="card p-6 space-y-5">
        <div className="pb-3 border-b border-line">
          ...
          <p className="text-xs text-ink-2 mt-0.5 leading-relaxed">Confronta come ripartisci il budget tra le categorie con budget “tipo” (esempi illustrativi). Non stima la probabilità di vincere.</p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-mute">Somma delle quote: <span className={`font-mono ${Math.abs(FIELDS.reduce((s, [k]) => s + draft[k], 0) - 1) < 0.005 ? 'text-emerald-700' : 'text-amber-700'}`}>{FIELDS.reduce((s, [k]) => s + draft[k], 0).toFixed(2)}</span> (dovrebbe fare 1)</p>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-field border border-line rounded-xl p-5 space-y-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="text-xs font-mono px-2 py-0.5 text-brand-ink rounded border border-brand/30">somiglianza {result.similarity_score.toFixed(2).replace('.', ',')}</span>
              </div>
            ...
            <div className="bg-field border border-line rounded-xl p-5 space-y-3">
              ...
              {FIELDS.map(([key, label]) => (
                <div key={key} className="space-y-1 text-xs">
                  <div className="flex justify-between gap-3 font-mono"><span className="text-ink-2">{label}</span><span className="text-brand-ink">tipo {Math.round(result.archetype_averages[key] * 100)}% · tu {Math.round(draft[key] * 100)}%</span></div>
