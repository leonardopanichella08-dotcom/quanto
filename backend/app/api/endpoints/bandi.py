...
from app.api.deps import actor_of, require_auth
from app.core import bandi, events, research, webhooks
...
def _source_refs(bando_id: str) -> List[tuple]:
    """(riferimento leggibile, testo) di tutte le fonti in memoria, le ufficiali per prime: base comune per l'estrazione."""
    b = Ingestion.get_bando(bando_id)
    name = b["name"] if b else ""
    out = []
    for s in events.list_bando_sources(bando_id):
        text = research.focus_text(s["text"], name)   # documenti lunghi: solo i passaggi che parlano del bando
        if not text.strip():
            continue
        label = s["name"] + (f" — {s['url']}" if s.get("url") else "") + (" (fonte secondaria)" if s.get("tier") == "SECONDARIA" else "")
        out.append((label[:300], text))
    return out
...
def research_search(body: ResearchSearch, request: Request) -> dict:
    ...
    try:
        result = research.search_web(body.name, body.hint, body.urls)
...
def research_fetch(body: ResearchFetch, request: Request) -> dict:
    ...
    links = research.find_links(doc["url"], doc["links"], known)
    ...
    return {"source": {"name": doc["title"], "url": doc["url"], "tier": doc["tier"], "kind": doc["kind"], "chars": doc["chars"], "pages": doc["pages"],
                       "size_bytes": doc["size_bytes"], "sha256": digest, "warnings": doc["warnings"]}, "links": links}
...
def research_analyze(body: ResearchAnalyze, request: Request, background: BackgroundTasks) -> dict:
    ...
    srcs = _source_refs(body.bando_id)
    if not srcs:
    ...
    outcome = Ingestion.extract(body.bando_id, sources=srcs)
    ...
    events.record("bando.research.analyze", f"Analizzate {len(srcs)} fonti: {len(outcome.published)} regole, {len(reqs)} requisiti ({len(to_review)} da rivedere), {len(detail['legal_refs'])} atti citati",
                  bando_id=body.bando_id, actor=actor_of(request), duration_ms=timer.ms,
                  details={"sources": len(srcs), "rules_published": sorted(outcome.published), "requirements": len(reqs), "to_review": len(to_review), "topics": topics})
    if outcome.coverage_activated:
        background.add_task(webhooks.emit, "event.bando.coverage_activated", {"bando_id": body.bando_id})
    return {"bando_id": body.bando_id, "sources": len(srcs), "rules_published": outcome.published, "requirements_total": len(reqs),
            "requirements_to_review": len(to_review), "topics": topics, "legal_refs": detail["legal_refs"], "detail": detail}


@router.get("/{bando_id}/sources/{sha256}", summary="Testo integrale di una fonte salvata in memoria")
...
def upload(body: UploadRequest, request: Request, background: BackgroundTasks) -> dict:
    ...
    digest = events.save_bando_source(bando_id, body.filename, text, url=body.source_url, origin="UPLOAD")
    events.add_document(kind, body.filename, raw, bando_id=bando_id, meta={"chars": len(text)})
    Ingestion.confirm(bando_id)
    outcome = Ingestion.extract(bando_id, sources=_source_refs(bando_id))
